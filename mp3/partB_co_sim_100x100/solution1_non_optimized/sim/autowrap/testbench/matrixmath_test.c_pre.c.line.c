...
#pragma empty_line
 for (rowC = 0;rowC<mA;rowC++)
  for (colC = 0;colC<nB;colC++)
  {
   ...
   for (colA = 0;colA<nA;colA++)
   {
    C[rowC*50 +colC]+=A[rowC*50 +colA]*B[colA*50 +colC];
   }
  }
#pragma empty_line
}
...
int main()
{
 int array_dim[3] = {10,50,50};
 int *A, *B;
 long *SW_C, *HW_C;
 A = (int*)malloc(sizeof(int)*50*50);
 B = (int*)malloc(sizeof(int)*50*50);
 SW_C = (long*)malloc(sizeof(long)*50*50);
 HW_C = (long*)malloc(sizeof(long)*50*50);
 long err_count;
...
#pragma empty_line
    ...
    for (rowC=0;rowC<mA;rowC++)
     for (colC=0;colC<nB;colC++)
      {
       if (SW_C[rowC*50 +colC]!=HW_C[rowC*50 +colC])
       {
        err_count++;
       }
      }
