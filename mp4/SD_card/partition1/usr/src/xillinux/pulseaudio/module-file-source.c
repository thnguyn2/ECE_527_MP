...
#include "module-file-source-symdef.h"

PA_MODULE_AUTHOR("Eli Billauer, Xillybus Ltd.");
PA_MODULE_DESCRIPTION("UNIX file source");
...
static void thread_func(void *userdata) {
    ...
    for (;;) {
        ...
        if (u->source->thread_info.state == PA_SOURCE_RUNNING && pollfd->revents) {
            ...
            pa_assert(l != 0); /* EOF shouldn't happen. */
...
int pa__init(pa_module*m) {
    ...
    pa_proplist_setf(data.proplist, PA_PROP_DEVICE_DESCRIPTION, "Device file source %s", u->filename);
    ...
    if (!(u->thread = pa_thread_new("file-source", thread_func, u))) {
