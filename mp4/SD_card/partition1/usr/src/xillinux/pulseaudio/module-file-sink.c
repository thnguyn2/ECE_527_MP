...
#include "module-file-sink-symdef.h"

PA_MODULE_AUTHOR("Eli Billauer, Xillybus Ltd.");
PA_MODULE_DESCRIPTION("UNIX file sink");
...
int pa__init(pa_module*m) {
    ...
    if ((u->fd = pa_open_cloexec(u->filename, O_WRONLY, 0)) < 0) {
    ...
    pa_proplist_setf(data.proplist, PA_PROP_DEVICE_DESCRIPTION, "Device file sink %s", u->filename);
    ...
    if (!(u->thread = pa_thread_new("file-sink", thread_func, u))) {
