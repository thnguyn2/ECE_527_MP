...
def get_avatar_path(url):
  ...
  avatar_cache_image = join(avatar_cache_dir, sha1(url).hexdigest())

  if not exists(avatar_cache_image) or len(open(avatar_cache_image, "r").read()) < 1:
    logger.debug("Downloading avatar %s", url)
...
def del_avatar(avatar):
  if exists(avatar):
    try:
      ...
      logger.error("Failed to remove avatar from cache: %s", avatar)
...
def get_ui_asset(asset_name):
  ...
  for base in DATA_DIRS:
    asset_path = os.path.join(base, 'data', asset_name)
    if os.path.exists(asset_path):
      return asset_path
...
def dump(service, aid, data):
  if GWIBBER_TEST_DUMP and len(data) > 0:
    ...
    if not exists(dump_cache_file) or len(open(dump_cache_file, "r").read()) < 1:
      logger.debug("Dumping test data %s - %s - %s", service, aid, operation)
