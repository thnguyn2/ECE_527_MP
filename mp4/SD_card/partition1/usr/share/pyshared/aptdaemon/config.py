...
class ConfigWriter(object):
    ...
    def parse(self, lines):
        ...
        for line_no, line in enumerate(lines):
            for char_no, char in enumerate(line):
                if in_comment and char == "/" and prev_char == "*":
    ...
    def set_value(self, option, value, defaultfile):
        ...
        etc_dir_parts = os.path.join(apt_pkg.config.find_dir("Dir::Etc"),
                                     apt_pkg.config.find_dir("Dir::Etc::Parts"))
        for filename in os.listdir(etc_dir_parts):
            if filename < defaultfile:
                continue
            with open(os.path.join(etc_dir_parts, filename)) as fd:
                lines = fd.readlines()
            ...
            try:
                ...
                if ((value == "true" and
                     val.string.lower() in ["yes", "with", "on", "enable"]) or
                    (value == "false" and
            ...
            with open(os.path.join(etc_dir_parts, filename), "w") as fd:
                log.debug("Writting %s", filename)
                fd.writelines(lines)
        if not os.path.exists(os.path.join(etc_dir_parts, defaultfile)):
            with open(os.path.join(etc_dir_parts, defaultfile), "w") as fd:
                log.debug("Writting %s", filename)
                line = "%s '%s';\n" % (option, value)
                fd.write(line)
