...
class TwistedTimestampChecker(utils.BaseTimestampChecker):
    ...
    def get_server_date_header(self, server_url):
        ...
        response = yield agent.request("HEAD", server_url, headers)
        defer.returnValue(response.headers.getRawHeaders("Date")[0])


tx_timestamp_checker = TwistedTimestampChecker()
...
class StorageClient(request.RequestHandler):
    ...
    def oauth_authenticate(self, consumer, token, metadata=None):
        ...
        """
        timestamp = yield tx_timestamp_checker.get_faithful_time()
