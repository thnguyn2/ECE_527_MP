...
def get_xdg_data_dirs():
    ...
    result.extend([x for x in get_env_path(
                'XDG_DATA_DIRS', default_data_path).split(os.pathsep)])
    return result
...
xdg_config_dirs = filter(lambda x: x, get_xdg_config_dirs())
xdg_data_dirs = filter(lambda x: x, get_xdg_data_dirs())
# pylint: disable=C0103
