...
def gethostname():
    ...
    hostname = socket.gethostname().decode(sys_encoding)
    return hostname
...
def get_old_token_name(app_name):
    ...
    assert isinstance(computer_name, unicode)
    assert isinstance(quoted_computer_name, unicode)

    return u"%s - %s" % (quoted_app_name, quoted_computer_name)
