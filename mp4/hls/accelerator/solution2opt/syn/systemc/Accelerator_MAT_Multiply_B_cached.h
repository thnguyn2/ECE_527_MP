...
#ifndef _Accelerator_MAT_Multiply_B_cached_H_
#define _Accelerator_MAT_Multiply_B_cached_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Accelerator_MAT_Multiply_B_cached_memcore.h"

SC_MODULE(Accelerator_MAT_Multiply_B_cached) {
...
protected:
    ...
    // buffer instances
    Accelerator_MAT_Multiply_B_cached_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(Accelerator_MAT_Multiply_B_cached) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "Accelerator_MAT_Multiply_B_cached_memcore_U_";
            name += szTmp;

            buffer[i] = new Accelerator_MAT_Multiply_B_cached_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Accelerator_MAT_Multiply_B_cached.trace");
            // ports
