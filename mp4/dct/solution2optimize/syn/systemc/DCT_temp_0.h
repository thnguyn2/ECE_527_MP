...
#ifndef _DCT_temp_0_H_
#define _DCT_temp_0_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "DCT_temp_0_memcore.h"

SC_MODULE(DCT_temp_0) {
...
protected:
    ...
    // buffer instances
    DCT_temp_0_memcore* buffer[BufferCount];
...
public:
    ~DCT_temp_0();
    SC_CTOR(DCT_temp_0) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "DCT_temp_0_memcore_U_";
            name += szTmp;

            buffer[i] = new DCT_temp_0_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "DCT_temp_0.trace");
            // ports
