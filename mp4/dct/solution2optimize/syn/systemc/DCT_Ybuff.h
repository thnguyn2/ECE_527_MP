...
#ifndef _DCT_Ybuff_H_
#define _DCT_Ybuff_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "DCT_Ybuff_memcore.h"
...
protected:
    ...
    DCT_Ybuff_memcore* DCT_Ybuff_memcore_U;
...
public:
    ~DCT_Ybuff();
    SC_CTOR(DCT_Ybuff) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        DCT_Ybuff_memcore_U = new DCT_Ybuff_memcore("DCT_Ybuff_memcore_U");
        DCT_Ybuff_memcore_U->clk      ( clk );
        DCT_Ybuff_memcore_U->reset    ( reset );
        DCT_Ybuff_memcore_U->ce0      ( i_ce0 );
        DCT_Ybuff_memcore_U->we0      ( i_we0 );
        DCT_Ybuff_memcore_U->address0 ( memcore_iaddr );
        DCT_Ybuff_memcore_U->d0       ( i_d0 );
        DCT_Ybuff_memcore_U->q0       ( i_q0 );

        DCT_Ybuff_memcore_U->ce1      ( t_ce0 );
        DCT_Ybuff_memcore_U->we1      ( t_we0 );
        DCT_Ybuff_memcore_U->address1 ( memcore_taddr );
        DCT_Ybuff_memcore_U->d1       ( t_d0 );
        DCT_Ybuff_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "DCT_Ybuff.trace");
            // ports
